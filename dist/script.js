...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_drop__WEBPACK_IMPORTED_MODULE_13__["default"])(caclState);
});
...
var burger = function burger(menuSelector, burgerSelector) {
  var menuElement = document.querySelector(menuSelector),
      burgerElement = document.querySelector(burgerSelector); // Скрываем бургер меню

  menuElement.style.display = 'none'; // Открытие меню при ширине экрана в 992 пикселей и ниже
...
var calc = function calc(size, material, options, promocod, result, state) {
  ...
  var sum = 0; // Функция расчета суммы

  var calcFunc = function calcFunc() {
    // Подсчет суммы
    sum = Math.round(+sizeBlock.value * +materialBlock.value + +optionsBlock.value); // Обязательнный выбор первых 2-х пунктов и если введен промокод, предоставить скидку
  ...
  ['drop', 'input'].forEach(function (eventName) {
    fileInputs.forEach(function (input) {
      input.addEventListener(eventName, function (e) {
        ...
        console.log(input.nodeName); // Отправка файлов на сервер сразу после drop/input (только в определенном блоке)
        ...
        if (input.getAttribute('data-upload')) {
          ...
          _toConsumableArray(input.files).forEach(function (file) {
            ...
            Object(_services_services__WEBPACK_IMPORTED_MODULE_14__["postData"])('assets/server.php', formData).then(function (res) {
              console.log(res);
...
var filter = function filter() {
  var menu = document.querySelector('.portfolio-menu'),
      ...
      no = document.querySelector('.portfolio-no'); // Фильтрация эл-тов
  ...
  function typeFilter(markType) {
    ...
    no.classList.remove('animated', 'fadeIn'); // Фильтр по категории
...
var forms = function forms(formSelector, state) {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots; // Разбиваем название файла на строки в массив

      var arr = item.files[0].name.split('.'); // Добавляем "..." если название больше 6 символов

      arr[0].length > 6 ? dots = "..." : dots = '.'; // Собираем редактированное название файла

      var name = arr[0].substring(0, 6) + dots + arr[1]; // Устанавливаем название файла
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault(); // Создание и добавление блока с уведомлением для пользователя

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      form.parentNode.appendChild(statusMessage); // Добавление анимации

      form.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        form.style.display = 'none';
      }, 400); // Создание изображения для отображения статуса отправки

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg); // Создание текста для отображения статуса отправки
...
var modal = function modal(triggerSelector, modalSelector, closeModalSelector) {
  ...
  var modalTrigger = document.querySelectorAll(triggerSelector),
      ...
      scroll = calcScroll();
  var btnPressed; // Открытие модального окна по кнопкам
...
var showModalByTime = setTimeout(function () {
  ...
  document.body.style.marginRight = "".concat(scroll, "px");
}, 60000); // Открытие модального окна при достижении футера
...
var pictureSize = function pictureSize(imgSelector) {
  var blocks = document.querySelectorAll(imgSelector); // Показ картинки
...
var scrolling = function scrolling(upSelector) {
  var upElem = document.querySelector(upSelector); // Показ "стрелочки" при прокрутке определенного количества пикселей страницы
...
var showMoreStyle = function showMoreStyle(trigger, wrapper) {
  var btn = document.querySelector(trigger); // // Добавление анимации ко всем карточкам
  // cards.forEach(card => {
  ...
  btn.addEventListener('click', function () {
    Object(_services_services__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/styles').then(function (res) {
